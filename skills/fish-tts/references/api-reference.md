...
| `s1` | 更早一代，情绪用 `(圆括号)` 英文固定标签（happy/sad/angry…） | 需要圆括号情绪控制、且只用固定英文标签 |

- 模型通过请求头 `model` 指定（不是 body）；省略或无法识别时回退 `s2.1-pro`。
- 多说话人对话合成仅 S2 系列（`s2-pro` / `s2.1-pro` / `s2.1-pro-free`）支持，`s1` 不支持。
- **情绪控制：S2 系列也支持**，用 `[方括号]` 且接受自由自然语言（中英文皆可）；S1 用 `(圆括号)` 固定英文标签。完整情绪参考见 `references/emotions.md`。
- `s1` 情绪示例：`今天天气真不错(happy)`（固定英文标签，**不要用中文 `(高兴)`**）。
